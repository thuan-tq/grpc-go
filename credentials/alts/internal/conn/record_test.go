...
// TestWriteFrameCount verifies that the number of encrypted frames written to the
// wire matches the negotiated frame size. With 1 MB frames, each payload fits in
// a single frame instead of being split into hundreds of 4 KB frames.
func (s) TestWriteFrameCount(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"1MB frames, 1 frame", altsWriteBufferMaxSize, 1, 1},
		{"1MB frames, 2 frames", altsWriteBufferMaxSize, 2, 2},
