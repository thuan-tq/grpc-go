...
func NewConn(c net.Conn, side core.Side, recordProtocol string, key []byte, protected []byte) (net.Conn, error) {
	...
	log.Printf("[ALTS-DEBUG] NewConn: protocol=%s side=%v overhead=%d payloadLengthLimit=%d altsRecordDefaultLength=%d altsWriteBufferMaxSize=%d altsReadBufferInitialSize=%d",
		recordProtocol, side, overhead, payloadLengthLimit, altsRecordDefaultLength, altsWriteBufferMaxSize, altsReadBufferInitialSize)
	return altsConn, nil
}
